...
A small, interactive CLI (Command Line Interface) tool that converts PNG image sequences into **WEBM / MP4 / GIF** using `ffmpeg`.
...
- ✅ Converts PNG frame sequences to:
    ...
    - **MP4 (H.264)** – standard video, no alpha support
    - **GIF** – 8-bit palette created using `palettegen` / `paletteuse`
...
- **Local Binary**
  ...
  Place the `ffmpeg.exe` file in the **same folder** as `render_cli.exe`.
